...
export default function ProductList({ products }) {
  const itemList = products.map((product) => (
    <li
      ...
      className="w-1/3 p-2 transition duration-300 ease-in-out hover:scale-110">
      <Link
        to={`/details/${product.productId}`}
        className=" bg-white h-full drop-shadow-sm flex flex-col justify-between items-center rounded py-4 px-2">
        <div className="m-2">
